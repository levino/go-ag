// Partieprotokoll 19×19 – Go/Baduk/Weiqi
...
#let lw          = 0.2mm
#let board       = 19
#let col-labels  = ("A","B","C","D","E","F","G","H","J","K","L","M","N","O","P","Q","R","S","T")
...
#grid(
  ...
  // 1: Metadaten
  titlebox(
    [#text(size: 13pt, weight: "bold")[Partieprotokoll — 19×19]],
    grid(
      columns: (1fr, 1fr, 1fr),
      inset: .4em,
  ...
  layout(size => {
    let sp-h = size.height / 20.2
    let sp-w = size.width  / 20.0
  ...
  titlebox(
    ...
    stack(spacing: 1.5em,
      v(0.6em),
      ..range(3).map(_ => line(length: 100%, stroke: (dash: "dotted", paint: text-col)))
    )
  ),
)
